...
async def process_message(message: aio_pika.IncomingMessage, id=None, cmd=None):
    """
    ...
    async with message.process():
        ...
        try:
            ...
            logger.error(f"[Consumer id:{id}] Error process", exc_info=True)
            return 1
...
async def start_consumer(queue_name, amqp_url, id):
    ...
    await queue.consume(functools.partial(process_message, id=id, cmd=queue_name))

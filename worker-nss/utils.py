...
def get_arp(ip: str, vlan: str, mode: int, as_ip: str) -> bool | str:
    ...
    match vlan:
        case 'INET':
            try:
                b.connect(hostname=as_ip, username="shchapov", pkey=key, port=22, timeout=5,disabled_algorithms={'pubkeys': ['rsa-sha2-512', 'rsa-sha2-256']})
        ...
        case 'fake':
            try:
                b.connect(hostname="109.95.48.4", username="shchapov", pkey=key, port=13883, timeout=5)
